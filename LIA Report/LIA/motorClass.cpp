...
void motorClass::backward(int x) {
  move(0, speed / 2, 0, speed / 2);  // Move the motors backward using speed/2 as the PWM values
}

void motorClass::left(int x) {
  move(1, speed / 3, 0, speed / 3);  // Turn the motors left using speed/3 as the PWM values
}

void motorClass::right(int x) {
  move(0, speed / 3, 1, speed / 3);  // Turn the motors right using speed/3 as the PWM values
}

void motorClass::forwardLeft(int x) {
  move(1, speed / 2, 1, speed / 4);  // Move diagonally forward-left using appropriate PWM values
}

void motorClass::forwardRight(int x) {
  move(1, speed / 4, 1, speed / 2);  // Move diagonally forward-right using appropriate PWM values
}

void motorClass::backwardLeft(int x) {
  move(0, speed / 2, 0, speed / 4);  // Move diagonally backward-left using appropriate PWM values
}

void motorClass::backwardRight(int x) {
  move(0, speed / 4, 0, speed / 2);  // Move diagonally backward-right using appropriate PWM values
}
